...
def main():
    ...
    def on_button(button, pressed):
        ...
        global left, right, forward, backward, running, img_count
    ...
    def on_axis(axis, value):
        ...
        if not backward and axis == "right_trigger":
            if value >= 0.5:
                forward = True
            else:
                forward = False
    ...
    while running:
        j.dispatch_events()
        print(f"Left:{left}, Right:{right}, Forward:{forward}, Backward:{backward}")
        client.set_control(int(forward), int(backward), int(left), int(right))
        time.sleep(0.3)
